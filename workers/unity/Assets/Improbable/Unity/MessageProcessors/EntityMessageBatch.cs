...
namespace Improbable.Unity.MessageProcessors
{
    ...
    {
        private readonly Dictionary<uint, IEntityStateUpdate> states = new Dictionary<uint, IEntityStateUpdate>();
        ...
        public void Add(IEntityMessage msg)
        {
            ...
            var componentUpdate = msg as ToEngineComponentUpdate;
            if (componentUpdate != null)
            {
                MergeState(componentUpdate.StateUpdate);
            }
        }
        ...
        private void MergeState(IEntityStateUpdate stateUpdate)
        {
            ...
            states.TryGetValue(stateUpdate.ComponentId, out existingStateUpdate);
            if (existingStateUpdate == null)
            {
                states.Add(stateUpdate.ComponentId, stateUpdate);
            }
            else
            {
