...
namespace Improbable.Unity.Core
{
    public class EngineConfiguration : Configuration
    {
        ...
        private readonly ILog logger = LogManager.GetLogger(typeof(EngineConfiguration));
        ...
        private EngineConfiguration()
        {
            defaultvalues = new DictionaryConfiguration
            {
                ...
                { ConfigNames.PROJECT_DESCRIPTOR_PATH, "../../spatialos.json"},
...
#if UNITY_EDITOR
            // Read APP_NAME from the spatialos.json file, if not already specified.
            // This is only done in Editor mode, as we do not expect spatialos.json to exist outside of dev environment.
            if (!IsAppNameSet)
            {
                try
                {
                    defaultvalues[ConfigNames.APP_NAME] = ProjectDescriptor.Load(ProjectDescriptorPath).Name;
                }
                catch (Exception e)
                {
                    logger.ErrorFormat("Cannot read app name from spatialos.json file. You will not be able to connect to a deployment. Underlying error: {0}", e);
                }
            }
#endif
        }

        private static readonly EngineConfiguration instance = new EngineConfiguration();
        ...
        public string ProjectDescriptorPath
        {
            get { return GetConfigValue<string>(ConfigNames.PROJECT_DESCRIPTOR_PATH); }
            set { defaultvalues[ConfigNames.PROJECT_DESCRIPTOR_PATH] = value; }
        }
        ...
        /// <summary>
        ///     This specifies the name of the application. If not specified, it will be loaded from your project descriptor.
        /// </summary>
        public string AppName
        {
            ...
            set
            {
                //Printing a warning as we cannot make a setter Obsolete.
                logger.Warn("AppName is no longer required to be set manually in your Unity workers. This setter will be removed in 6.0.0");
                defaultvalues[ConfigNames.APP_NAME] = value;
            }
        }

        /// <summary>
        ///     Indicates whether the AppName has been autodiscovered or explicitly set.
        /// </summary>
        public bool IsAppNameSet
        {
            ...
            {
                string appName;
                return TryGetConfigValue<string>(ConfigNames.APP_NAME, out appName);
            }
        }
        ...
        public ILoadMetricProvider LoadMetricProvider { get; set; }
    }
}